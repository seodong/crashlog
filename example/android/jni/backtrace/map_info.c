...
static map_info_t *parse_vmmap_line(const char *line)
{
	...
	char permissions[4];
	int name_pos;
	if (sscanf(line, "%*21c %lx-%lx [%*13c] %3c/%*3c SM=%*3c  %n", &start, &end, permissions, &name_pos) != 3) {
		return NULL;
	}
...
const char *GetMapName(const map_info_t * milist, uintptr_t addr)
{
	const map_info_t *map_info = find_map_info(milist, addr);

	if (map_info) {
		//return strdup(map_info->name);

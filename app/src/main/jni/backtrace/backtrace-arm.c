#include "backtrace/backtrace-arch.h"
...
static uintptr_t get_exception_handler(const memory_t * memory, const map_info_t * map_info_list, uintptr_t pc)
{
	if (!pc) {
 ...
 */
static bool execute_personality_routine(const memory_t * memory, unwind_state_t * state, byte_stream_t * stream, int pr_index)
{
	size_t size;
	switch (pr_index) {
...
static ssize_t unwind_backtrace_common(const memory_t * memory,
				       ...
				       unwind_state_t * state, backtrace_frame_t * backtrace, size_t ignore_depth, size_t max_depth)
{
	...
	for (index = 0; returned_frames < max_depth; index++) {
		...
		backtrace_frame_t *frame = add_backtrace_entry(pc,
							       backtrace, ignore_depth, max_depth, &ignored_frames,
							       &returned_frames);
		if (frame) {
				     ...
				     backtrace_frame_t * backtrace, size_t ignore_depth, size_t max_depth)
{
	...
	return unwind_backtrace_common(&memory, map_info_list, &state, backtrace, ignore_depth, max_depth);
}

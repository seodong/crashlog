...
static int crashlog_path(char * fname, size_t fsize)
{
	char log_path[256];
	...
	snprintf(fname, fsize - 1, "%s/crash.txt", log_path);
	return 1;
}
...
static void signal_segv(int signum, siginfo_t * info, void *ptr)
{
	...
	if(crashlog_path(fname, sizeof(fname)) >0)
			LOGD("Write crashlog in %s\n",fname);
	exit(-1);
